...
### Added
- `--auto` flag for `/gsd-teams:new-project` — runs research → requirements → roadmap automatically after config questions. Expects idea document via @ reference (e.g., `/gsd-teams:new-project --auto @prd.md`)
...
- Research decision from `/gsd-teams:new-milestone` now persists to config.json

## [1.17.0] - 2026-02-08

### Added
- **gsd-teams-tools verification suite**: `verify plan-structure`, `verify phase-completeness`, `verify references`, `verify commits`, `verify artifacts`, `verify key-links` — deterministic structural checks
- **gsd-teams-tools frontmatter CRUD**: `frontmatter get/set/merge/validate` — safe YAML frontmatter operations with schema validation
- **gsd-teams-tools template fill**: `template fill summary/plan/verification` — pre-filled document skeletons
- **gsd-teams-tools state progression**: `state advance-plan`, `state update-progress`, `state record-metric`, `state add-decision`, `state add-blocker`, `state resolve-blocker`, `state record-session` — automates STATE.md updates
- **Local patch preservation**: Installer now detects locally modified GSD files, backs them up to `gsd-teams-local-patches/`, and creates a manifest for restoration
- `/gsd-teams:reapply-patches` command to merge local modifications back after GSD updates

### Changed
- Agents (executor, planner, plan-checker, verifier) now use gsd-teams-tools for state updates and verification instead of manual markdown parsing
- `/gsd-teams:update` workflow now notifies about backed-up local patches and suggests `/gsd-teams:reapply-patches`
...
### Added
- 10 new gsd-teams-tools CLI commands that replace manual AI orchestration of mechanical operations:
...
### Changed
- Workflows now delegate deterministic operations to gsd-teams-tools CLI, reducing token usage and errors:
...
### Fixed
- Execute-phase now correctly spawns `gsd-teams-executor` subagents instead of generic task agents
- `commit_docs=false` setting now respected in all `.planning/` commit paths (execute-plan, debugger, reference docs all route through gsd-teams-tools CLI)
...
- Windows: Normalized backslash paths in gsd-teams-tools invocations (contributed by @rmindel)
...
### Added
- Context-optimizing parsing commands in gsd-teams-tools (`phase-plan-index`, `state-snapshot`, `summary-extract`) — reduces agent context usage by returning structured JSON instead of raw file content
...
### Added
- `gsd-teams-tools history-digest` — Compiles phase summaries into structured JSON for faster context loading
- `gsd-teams-tools phases list` — Lists phase directories with filtering (replaces fragile `ls | sort -V` patterns)
- `gsd-teams-tools roadmap get-phase` — Extracts phase sections from ROADMAP.md
- `gsd-teams-tools phase next-decimal` — Calculates next decimal phase number for insert operations
- `gsd-teams-tools state get/patch` — Atomic STATE.md field operations
- `gsd-teams-tools template select` — Chooses summary template based on plan complexity
...
- Agents migrated from bash patterns to structured gsd-teams-tools commands
...
- **Centralized utilities** — New `gsd-teams-tools.js` (11 functions) replaces repetitive bash patterns across 50+ files
...
### Added
- `gsd-teams-tools.js` CLI utility with functions: state load/update, resolve-model, find-phase, commit, verify-summary, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section
...
### Fixed
- **CRITICAL:** Prevent API keys from being committed via `/gsd-teams:map-codebase` (#429)
...
- Auto-create `config.json` when missing during `/gsd-teams:settings` (#264)
- `/gsd-teams:update` respects local vs global install location
...
### Fixed
- CONTEXT.md from `/gsd-teams:discuss-phase` now properly flows to all downstream agents (researcher, planner, checker, revision loop)
...
### Removed
- `/gsd-teams:whats-new` command — use `/gsd-teams:update` instead (shows changelog with cancel option)
...
### Added
- `/gsd-teams:join-discord` command to quickly access the GSD Discord community invite link
...
- OpenCode commands use flat structure (`command/gsd-teams-help.md`) matching OpenCode's expected format
...
- Removed stray backticks in `/gsd-teams:new-project` output
...
- **Codebase Intelligence System** — Removed due to overengineering concerns
  - Deleted `/gsd-teams:analyze-codebase` command
  - Deleted `/gsd-teams:query-intel` command
  - Removed SQLite graph database and sql.js dependency (21MB)
  - Removed intel hooks (gsd-teams-intel-index.js, gsd-teams-intel-session.js, gsd-teams-intel-prune.js)
...
### Added
- **Model Profiles** — `/gsd-teams:set-profile` for quality/balanced/budget agent configurations
- **Workflow Settings** — `/gsd-teams:settings` command for toggling workflow behaviors interactively
...
- All hooks now use `gsd-` prefix for consistency (statusline.js → gsd-teams-statusline.js)
...
- `/gsd-teams:new-project` now asks about git tracking during initial setup, letting you opt out of committing planning docs from the start
...
### Added
- **Quick Mode** (`/gsd-teams:quick`) — Execute small, ad-hoc tasks with GSD guarantees but skip optional agents (researcher, checker, verifier). Quick tasks live in `.planning/quick/` with their own tracking in STATE.md.
...
- Orphaned `gsd-teams-notify.sh` hook from previous versions is now automatically removed during install (both file and settings.json registration)

## [1.6.3] - 2025-01-17

### Added
- `--gaps-only` flag for `/gsd-teams:execute-phase` — executes only gap closure plans after verify-work finds issues, eliminating redundant state discovery
...
- `/gsd-teams:update` shows changelog and asks for confirmation before updating, with clear warning about what gets replaced

## [1.6.0] - 2026-01-17

### Changed
- **BREAKING:** Unified `/gsd-teams:new-milestone` flow — now mirrors `/gsd-teams:new-project` with questioning → research → requirements → roadmap in a single command
- Roadmapper agent now references templates instead of inline structures for easier maintenance

### Removed
- **BREAKING:** `/gsd-teams:discuss-milestone` — consolidated into `/gsd-teams:new-milestone`
- **BREAKING:** `/gsd-teams:create-roadmap` — integrated into project/milestone flows
- **BREAKING:** `/gsd-teams:define-requirements` — integrated into project/milestone flows
- **BREAKING:** `/gsd-teams:research-project` — integrated into project/milestone flows

### Added
- `/gsd-teams:verify-work` now includes next-step routing after verification completes
...
- Next-step suggestions now consistently recommend `/gsd-teams:discuss-phase` before `/gsd-teams:plan-phase` across all routing paths
...
- Removed blocking notification popups (gsd-teams-notify) on all platforms
...
- **BREAKING:** Removed `/gsd-teams:execute-plan` command (use `/gsd-teams:execute-phase` instead)
...
- Removed `/gsd-teams:research-phase` from next-step suggestions (use `/gsd-teams:plan-phase` instead)

## [1.5.22] - 2025-01-16

### Added
- Statusline update indicator — shows `⬆ /gsd-teams:update` when a new version is available
...
### Changed
- **Unified `/gsd-teams:new-project` flow** — Single command now handles questions → research → requirements → roadmap (~10 min)
- Simplified README to reflect streamlined workflow: new-project → plan-phase → execute-phase
- Added optional `/gsd-teams:discuss-phase` documentation for UI/UX/behavior decisions before planning
...
### Changed
- `/gsd-teams:discuss-phase` redesigned with intelligent gray area analysis — analyzes phase to identify discussable areas (UI, UX, Behavior, etc.), presents multi-select for user control, deep-dives each area with focused questioning
...
- `/gsd-teams:plan-phase` now integrates research — spawns `gsd-teams-phase-researcher` before planning unless research exists or `--skip-research` flag used

## [1.5.18] - 2026-01-16

### Added
- **Plan verification loop** — Plans are now verified before execution with a planner → checker → revise cycle
  - New `gsd-teams-plan-checker` agent (744 lines) validates plans will achieve phase goals
...
- **Dedicated planner agent** — `gsd-teams-planner` (1,319 lines) consolidates all planning expertise
...
### Changed
- `/gsd-teams:plan-phase` refactored to thin orchestrator pattern (310 lines)
  - Spawns `gsd-teams-planner` for planning, `gsd-teams-plan-checker` for verification
  - User sees status between agent spawns (not a black box)
- Planning references deprecated with redirects to `gsd-teams-planner` agent sections
...
### Added
- New `/gsd-teams:update` command — check for updates, install, and display changelog of what changed (better UX than raw `npx get-shit-done-cc-teams`)
...
- New `gsd-teams-debugger` agent (990 lines) with scientific debugging methodology, hypothesis testing, and 7+ investigation techniques
- New `gsd-teams-codebase-mapper` agent for brownfield codebase analysis
- Research subagent prompt template for context-only spawning

### Changed
- `/gsd-teams:research-phase` refactored to thin orchestrator — now injects rich context (key insight framing, downstream consumer info, quality gates) to gsd-researcher agent
- `/gsd-teams:research-project` refactored to spawn 4 parallel gsd-researcher agents with milestone-aware context (greenfield vs v1.1+) and roadmap implications guidance
- `/gsd-teams:debug` refactored to thin orchestrator (149 lines) — spawns gsd-teams-debugger agent with full debugging expertise
- `/gsd-teams:new-milestone` now explicitly references MILESTONE-CONTEXT.md
...
- `workflows/debug.md` — consolidated into gsd-teams-debugger agent
- `references/research-pitfalls.md` — consolidated into gsd-researcher agent
- `references/debugging.md` — consolidated into gsd-teams-debugger agent
- `references/debug-investigation.md` — consolidated into gsd-teams-debugger agent

## [1.5.15] - 2025-01-15

### Fixed
- **Agents now install correctly** — The `agents/` folder (gsd-teams-executor, gsd-teams-verifier, gsd-teams-integration-checker, gsd-milestone-auditor) was missing from npm package, now included

### Changed
- Consolidated `/gsd-teams:plan-fix` into `/gsd-teams:plan-phase --gaps` for simpler workflow
- UAT file writes now batched instead of per-response for better performance

## [1.5.14] - 2025-01-15

### Fixed
- Plan-phase now always routes to `/gsd-teams:execute-phase` after planning, even for single-plan phases

## [1.5.13] - 2026-01-15

### Fixed
- `/gsd-teams:new-milestone` now presents research and requirements paths as equal options, matching `/gsd-teams:new-project` format
...
- `progress` now correctly routes to `/gsd-teams:discuss-milestone` when between milestones (Route F)
...
### Added
- Milestone audit system (`/gsd-teams:audit-milestone`) for verifying milestone completion with parallel verification agents
...
- Execute-phase now recommends `/gsd-teams:audit-milestone` when milestone completes
...
### Changed
- `gsd-teams-executor` subagent color changed from red to blue

## [1.5.7] - 2025-01-15

### Added
- `gsd-teams-executor` subagent: Dedicated agent for plan execution with full workflow logic built-in
- `gsd-teams-verifier` subagent: Goal-backward verification that checks if phase goals are actually achieved (not just tasks completed)
...
### Changed
- execute-plan and execute-phase now spawn `gsd-teams-executor` subagent instead of using inline workflow
...
### Fixed
- Updated remaining `general-purpose` subagent references to use `gsd-teams-executor`
...
### Removed
- Dead `/gsd-teams:status` command (referenced abandoned background agent model)
...
### Added
- New `/gsd-teams:research-project` command for pre-roadmap ecosystem research — spawns parallel agents to investigate stack, features, architecture, and pitfalls before you commit to a roadmap
- New `/gsd-teams:define-requirements` command for scoping v1 requirements from research findings — transforms "what exists in this domain" into "what we're building"
...
### Added
- New `/gsd-teams:whats-new` command shows changes since your installed version
...
### Added
- `/gsd-teams:debug` command for systematic debugging with persistent state
...
### Added
- Parallel phase execution via `/gsd-teams:execute-phase`
- Parallel-aware planning in `/gsd-teams:plan-phase`
- `/gsd-teams:status` command for parallel agent monitoring
...
### Added
- `/gsd-teams:add-todo` and `/gsd-teams:check-todos` for mid-session idea capture
...
### Added
- `/gsd-teams:resume-task` for resuming interrupted subagent executions
...
### Added
- `/gsd-teams:verify-work` for conversational UAT validation
- `/gsd-teams:plan-fix` for fixing UAT issues
- UAT issues template
...
- `/gsd-teams:remove-phase` command
...
### Added
- `/gsd-teams:map-codebase` documentation in help and README

## [1.3.0] - 2025-12-17

### Added
- `/gsd-teams:map-codebase` command for brownfield project analysis
...
- `/gsd-teams:research-phase` for niche domain ecosystem discovery
- `/gsd-teams:research-project` command with workflow and templates
- `/gsd-teams:create-roadmap` command with research-aware workflow
...
- Core slash commands: `/gsd-teams:new-project`, `/gsd-teams:discuss-phase`, `/gsd-teams:plan-phase`, `/gsd-teams:execute-phase`
- PROJECT.md and STATE.md templates
- Phase-based development workflow
- YOLO mode for autonomous execution
- Interactive mode with checkpoints

[Unreleased]: https://github.com/glittercowboy/get-shit-done-teams/compare/v1.18.0...HEAD
[1.18.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.18.0
[1.17.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.17.0
[1.16.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.16.0
[1.15.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.15.0
[1.14.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.14.0
[1.13.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.13.0
[1.12.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.12.1
[1.12.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.12.0
[1.11.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.11.2
[1.11.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.11.0
[1.10.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.10.1
[1.10.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.10.0
[1.9.12]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.12
[1.9.11]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.11
[1.9.10]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.10
[1.9.9]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.9
[1.9.8]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.8
[1.9.7]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.7
[1.9.6]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.6
[1.9.5]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.5
[1.9.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.4
[1.9.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.2
[1.9.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.9.0
[1.8.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.8.0
[1.7.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.7.1
[1.7.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.7.0
[1.6.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.6.4
[1.6.3]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.6.3
[1.6.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.6.2
[1.6.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.6.1
[1.6.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.6.0
[1.5.30]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.30
[1.5.29]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.29
[1.5.28]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.28
[1.5.27]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.27
[1.5.26]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.26
[1.5.25]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.25
[1.5.24]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.24
[1.5.23]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.23
[1.5.22]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.22
[1.5.21]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.21
[1.5.20]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.20
[1.5.19]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.19
[1.5.18]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.18
[1.5.17]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.17
[1.5.16]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.16
[1.5.15]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.15
[1.5.14]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.14
[1.5.13]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.13
[1.5.12]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.12
[1.5.11]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.11
[1.5.10]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.10
[1.5.9]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.9
[1.5.8]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.8
[1.5.7]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.7
[1.5.6]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.6
[1.5.5]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.5
[1.5.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.4
[1.5.3]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.3
[1.5.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.2
[1.5.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.1
[1.5.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.5.0
[1.4.29]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.29
[1.4.28]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.28
[1.4.27]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.27
[1.4.26]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.26
[1.4.25]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.25
[1.4.24]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.24
[1.4.23]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.23
[1.4.22]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.22
[1.4.21]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.21
[1.4.20]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.20
[1.4.19]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.19
[1.4.18]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.18
[1.4.17]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.17
[1.4.16]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.16
[1.4.15]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.15
[1.4.14]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.14
[1.4.13]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.13
[1.4.12]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.12
[1.4.11]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.11
[1.4.10]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.10
[1.4.9]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.9
[1.4.8]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.8
[1.4.7]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.7
[1.4.6]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.6
[1.4.5]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.5
[1.4.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.4
[1.4.3]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.3
[1.4.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.2
[1.4.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.1
[1.4.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.4.0
[1.3.34]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.34
[1.3.33]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.33
[1.3.32]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.32
[1.3.31]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.31
[1.3.30]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.30
[1.3.29]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.29
[1.3.28]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.28
[1.3.27]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.27
[1.3.26]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.26
[1.3.25]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.25
[1.3.24]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.24
[1.3.23]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.23
[1.3.22]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.22
[1.3.21]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.21
[1.3.20]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.20
[1.3.19]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.19
[1.3.18]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.18
[1.3.17]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.17
[1.3.16]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.16
[1.3.15]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.15
[1.3.14]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.14
[1.3.13]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.13
[1.3.12]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.12
[1.3.11]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.11
[1.3.10]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.10
[1.3.9]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.9
[1.3.8]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.8
[1.3.7]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.7
[1.3.6]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.6
[1.3.5]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.5
[1.3.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.4
[1.3.3]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.3
[1.3.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.2
[1.3.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.1
[1.3.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.3.0
[1.2.13]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.13
[1.2.12]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.12
[1.2.11]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.11
[1.2.10]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.10
[1.2.9]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.9
[1.2.8]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.8
[1.2.7]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.7
[1.2.6]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.6
[1.2.5]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.5
[1.2.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.4
[1.2.3]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.3
[1.2.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.2
[1.2.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.1
[1.2.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.2.0
[1.1.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.1.2
[1.1.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.1.1
[1.1.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.1.0
[1.0.11]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.11
[1.0.10]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.10
[1.0.9]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.9
[1.0.8]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.8
[1.0.7]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.7
[1.0.6]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.6
[1.0.5]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.5
[1.0.4]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.4
[1.0.3]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.3
[1.0.2]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.2
[1.0.1]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.1
[1.0.0]: https://github.com/glittercowboy/get-shit-done-teams/releases/tag/v1.0.0

---
name: gsd-teams-debugger
description: Investigates bugs using scientific method, manages debug sessions, handles checkpoints. Spawned by /gsd-teams:debug orchestrator.
...
- `/gsd-teams:debug` command (interactive debugging)
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest "/clear - run /gsd-teams:debug to resume" if context filling up.
</step>
...
```bash
INIT=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state load)
...
```bash
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```

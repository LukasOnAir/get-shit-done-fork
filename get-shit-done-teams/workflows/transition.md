...
**Delegate ROADMAP.md and STATE.md updates to gsd-teams-tools:**

```bash
TRANSITION=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gsd-teams-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js progress bar --raw)
```
...
**Use the transition result from `gsd-teams-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/gsd-teams:plan-phase [X+1]")
...
**Also available:**
- `/gsd-teams:discuss-phase [X+1]` — gather context first
- `/gsd-teams:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/gsd-teams:complete-milestone {version}")

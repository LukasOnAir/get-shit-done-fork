...
@~/.claude/get-shit-done-teams/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js init execute-phase "${PHASE}" --include state,config)
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gsd-teams-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn gsd-teams-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
See `~/.claude/get-shit-done-teams/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/get-shit-done-teams/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/get-shit-done-teams/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/get-shit-done-teams/templates/summary.md`.
...
Update STATE.md using gsd-teams-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state advance-plan

# Recalculate progress bar from disk state
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state update-progress

# Record execution metrics
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state record-metric \
...
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state add-blocker "Blocker description"
```
</step>

<step name="update_session_continuity">
Update session info using gsd-teams-tools:

```bash
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state record-session \
...
```bash
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
</step>
...
```bash
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gsd-teams:execute-phase {phase}` + `/gsd-teams:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gsd-teams:plan-phase {Z+1}` + `/gsd-teams:verify-work {Z}` + `/gsd-teams:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gsd-teams:complete-milestone` + `/gsd-teams:verify-work` + `/gsd-teams:add-phase` |

<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gsd-teams-phase-researcher, gsd-teams-planner, and gsd-teams-plan-checker agents with a revision loop (max 3 iterations).
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@~/.claude/get-shit-done-teams/references/ui-brand.md
</required_reading>
...
```bash
INIT=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
**If `planning_exists` is false:** Error — run `/gsd-teams:new-project` first.
...
### Spawn gsd-teams-phase-researcher

```bash
PHASE_DESC=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js state-snapshot)
...
<phase_context>
IMPORTANT: If CONTEXT.md exists below, it contains user decisions from /gsd-teams:discuss-phase.
...
```
Task(
  prompt="First, read ~/.claude/agents/gsd-teams-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
Read ~/.claude/agents/gsd-teams-planner.md for your role and instructions.
</team_behavior>
```
...
## 8. Spawn gsd-teams-planner Agent
...
**Phase Context:**
IMPORTANT: If context exists below, it contains USER DECISIONS from /gsd-teams:discuss-phase.
...
<downstream_consumer>
Output consumed by /gsd-teams:execute-phase. Plans need:
...
```
Task(
  prompt="First, read ~/.claude/agents/gsd-teams-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
## 10. Spawn gsd-teams-plan-checker Agent
...
Task(
  ...
  subagent_type="gsd-teams-plan-checker",
  model="{checker_model}",
...
- /gsd-teams:plan-phase {X} --research — re-research first
...
- [ ] gsd-teams-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] gsd-teams-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gsd-teams-plan-checker spawned with CONTEXT.md

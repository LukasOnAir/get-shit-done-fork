...
```bash
timestamp=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
...
To resume: /gsd-teams:resume-work

...
```bash
INIT=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js init execute-phase "${PHASE_ARG}")
```
...
```bash
PLAN_INDEX=$(node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js phase-plan-index "${PHASE_NUMBER}")
```
...
@~/.claude/get-shit-done-teams/references/checkpoints.md
@~/.claude/get-shit-done-teams/references/tdd.md
</execution_context>
...
Check must_haves against actual codebase. Create VERIFICATION.md.",
  subagent_type="gsd-teams-verifier",
...
| `gaps_found` | Present gap summary, offer `/gsd-teams:plan-phase {phase} --gaps` |
...
`/gsd-teams:plan-phase {X} --gaps`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase}-VERIFICATION.md` — full report
Also: `/gsd-teams:verify-work {X}` — manual testing first
```

Gap closure cycle: `/gsd-teams:plan-phase {X} --gaps` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/gsd-teams:execute-phase {X} --gaps-only` → verifier re-runs.
</step>
...
```bash
node ~/.claude/get-shit-done-teams/bin/gsd-teams-tools.js commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/phases/{phase_dir}/*-VERIFICATION.md .planning/REQUIREMENTS.md
```
</step>
...
<resumption>
Re-run `/gsd-teams:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.

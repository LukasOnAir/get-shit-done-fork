 ...
 *
 * Usage: node gsd-teams-tools.js <command> [args] [--raw]
 *
 * Atomic Commands:
...
const MODEL_PROFILES = {
  'gsd-teams-planner':              { quality: 'opus', balanced: 'opus',   budget: 'sonnet' },
  'gsd-teams-roadmapper':           { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  'gsd-teams-executor':             { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  'gsd-teams-phase-researcher':     { quality: 'opus', balanced: 'sonnet', budget: 'haiku' },
  'gsd-teams-project-researcher':   { quality: 'opus', balanced: 'sonnet', budget: 'haiku' },
  'gsd-teams-research-synthesizer': { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
  'gsd-teams-debugger':             { quality: 'opus', balanced: 'sonnet', budget: 'sonnet' },
  'gsd-teams-codebase-mapper':      { quality: 'sonnet', balanced: 'haiku', budget: 'haiku' },
  'gsd-teams-verifier':             { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
  'gsd-teams-plan-checker':         { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
  'gsd-teams-integration-checker':  { quality: 'sonnet', balanced: 'sonnet', budget: 'haiku' },
};
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${newPhaseNum}: ${description}\n\n**Goal:** [To be planned]\n**Depends on:** Phase ${maxPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /gsd-teams:plan-phase ${newPhaseNum} to break down)\n`;
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /gsd-teams:plan-phase ${decimalPhase} to break down)\n`;
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during /gsd-teams:discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
...
function cmdInitExecutePhase(cwd, phase, includes, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'gsd-teams-executor'),
    verifier_model: resolveModelInternal(cwd, 'gsd-teams-verifier'),
...
function cmdInitPlanPhase(cwd, phase, includes, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gsd-teams-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'gsd-teams-planner'),
    checker_model: resolveModelInternal(cwd, 'gsd-teams-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gsd-teams-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'gsd-teams-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'gsd-teams-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'gsd-teams-codebase-mapper'),
...
async function main() {
  ...
  if (!command) {
    error('Usage: gsd-teams-tools <command> [args] [--raw]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
